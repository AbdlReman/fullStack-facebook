    ...
    const promises = imagePreviews.map(async (img) => {
      try {
        ...
        const res = await fetch(
          "https://api.cloudinary.com/v1_1/(add own cloudinary username)/image/upload",
          {
            method: "POST",
            body: data,
          }
      ...
      <Card>
        <Card.Header className="d-flex bg-white align-items-center gap-4">
          <img
            ...
            src="https://4.bp.blogspot.com/-23zvGymeTyA/XhV9fNzozsI/AAAAAAAAL2E/Gru5mTIARdAoiO4wVFAVOqv6K916UP89QCLcBGAsYHQ/s1600/Facebook-new-design.jpg"
            alt=""
          />
          ...
          <Form className="w-100 d-flex align-items-center">
            <Form.Control
              ...
              type="text"
              className="w-100 bg-light rounded-pill"
              placeholder={`What's on your mind ${user?.f_name}?`}
            />
            {postLoading ? (
              <SyncLoader />
            ) : (
              <IoIosSend onClick={postCaption} cursor="pointer" />
            )}
          </Form>
        </Card.Header>
        {postLoading ? (
